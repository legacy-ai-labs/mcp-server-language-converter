...
for rule in paragraph_rules[:20]:
    indent = "  " * rule['depth']
    print(f"{indent}{rule['rule_name']} (index={rule['rule_index']}, children={rule['children_count']})")

print("\n" + "=" * 80)
print("\nProgram name rules:")
print("=" * 80)
program_rules = [r for r in rule_names if 'program' in r['rule_name'].lower() and 'name' in r['rule_name'].lower()]
