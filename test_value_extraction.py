...
def find_paragraph_names(node, parser_instance, depth=0):
    ...
    if hasattr(node, "getRuleIndex"):
        ...
        if rule_name == "paragraphName":
            result = {"depth": depth, "rule_name": rule_name, "children": []}
            ...
            if node.children:
                for child in node.children:
                    if isinstance(child, TerminalNode):
                        ...
                        token_type = (
                            parser_instance.symbolicNames[symbol.type]
                            if symbol.type < len(parser_instance.symbolicNames)
                            else "UNKNOWN"
                        )
                        result["children"].append(
                            {"type": "Terminal", "token_type": token_type, "value": symbol.text}
                        )
