"""Generic STDIO transport runner for MCP servers.

This module provides a reusable STDIO server runner that can be used by any
domain-specific MCP server. It handles server initialization, tool loading,
and running the server with STDIO transport.
"""
...
def run_stdio_server(domain: str, server_name: str | None = None) -> None:
    ...
    try:
        ...
        print(f"MCP Server starting for domain: {domain}...", file=sys.stderr)
